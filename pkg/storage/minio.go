...
func (s *Storage) Reset() error {
	...
	go func() {
		...
		for object := range s.minio.ListObjects(s.ctx, s.cfg.Bucket, s.cfg.ListObjectsOptions) {
			if object.Err != nil {
				klog.Errorf("object %s", object.Err)
			}
			objectsCh <- object
		}
	}()
	...
	for err := range s.minio.RemoveObjects(s.ctx, s.cfg.Bucket, objectsCh, opts) {
		klog.Errorf("Error detected during deletion: %s", err)
	}
...
func (s *Storage) PresignedGetObject(bucketName, objectName string, expiry time.Duration) (string, error) {
	reqParams := make(url.Values)
	presignedURL, err := s.minio.PresignedGetObject(context.Background(), bucketName, objectName, expiry, reqParams)
	if err != nil {
		return "", err
	}
	return presignedURL.String(), nil
