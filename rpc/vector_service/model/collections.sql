...
COMMENT ON COLUMN service.info.name IS '服务名称';
...
CREATE TABLE IF NOT EXISTS service.collection (
    ...
    bounds GEOMETRY(Polygon, 4326), -- 集合的几何边界
    center GEOMETRY(Point, 4326), -- 集合的几何中心点
    srid INT, -- 几何的空间参考标识符
    service_category INT NOT NULL, -- 服务类别:mvt、feature、wmts
...
COMMENT ON COLUMN service.collection.name IS '集合名称';
COMMENT ON COLUMN service.collection.bounds IS '集合的几何边界';
...
-- 创建 GIST 索引
CREATE INDEX idx_collection_bounds ON service.collection USING GIST(bounds);
...
CREATE TABLE IF NOT EXISTS service.vector (
                                              ...
                                              service_key UUID NOT NULL UNIQUE, -- 服务的唯一键
                                              name VARCHAR(255) NOT NULL, -- 矢量服务名称
...
COMMENT ON COLUMN service.vector.service_key IS '服务的唯一键';
COMMENT ON COLUMN service.vector.name IS '矢量服务名称';
...
CREATE TABLE IF NOT EXISTS service.imagery (
                                               ...
                                               service_key UUID NOT NULL UNIQUE, -- 服务的唯一键
                                               name VARCHAR(255) NOT NULL, -- 影像服务名称
    created_at TIMESTAMP DEFAULT NOW(), -- 创建时间戳
    updated_at TIMESTAMP DEFAULT NOW(), -- 最后更新时间戳
    deleted_at TIMESTAMP DEFAULT NULL -- 软删除时间戳
    );
...
COMMENT ON COLUMN service.imagery.name IS '影像服务名称';
...
COMMENT ON COLUMN service.style.name IS '样式名称';
