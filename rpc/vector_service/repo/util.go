...
func getServerBounds() (b *Bounds, e error) {
	...
	klog.Infof("Using CoordinateSystem.SRID %d with bounds [%g, %g, %g, %g]",
...
func tileMetrics(h http.Handler) http.Handler {
	if viper.GetBool("EnableMetrics") {
		...
		basePath := viper.GetString("BasePath")
		klog.Infof("Prometheus metrics enabled at %s/metrics", formatBaseURL(fmt.Sprintf("http://%s:%d",
		...
		return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			...
			if err != nil {
				klog.Warn("Unable to get tilesProcessed Prometheus counter.")
				return
			}
			...
			if err != nil {
				klog.Warn("Unable to get tilesDurationHistogram Prometheus histogram.")
				return
			}
